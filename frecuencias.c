...
int Find_bin(float value, float min_meas, float max_meas, int bin_count) {
    ...
    if (bin == bin_count) bin--; // por si cae exactamente en max_meas
    return bin;
}
...
int main(int argc, char* argv[]) {
    ...
    // Datos de ejemplo (solo el root los tiene al inicio)
    ...
    // Histograma global (solo root recibe)
    int* bin_counts = NULL;
    if (rank == 0) {
        bin_counts = (int*)calloc(bin_count, sizeof(int));
    }

    // Reducir sumando los histogramas locales
    MPI_Reduce(loc_bin_counts, bin_counts, bin_count, MPI_INT,
               MPI_SUM, 0, MPI_COMM_WORLD);

    // Imprimir resultado (solo root)
    if (rank == 0) {
