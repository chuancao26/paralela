...
int main(int argc, char* argv[]) {
    ...
    // Solo el proceso 0 pide el número de lanzamientos
    if (rank == 0) {
    ...
    // Se comparte el total con todos
    MPI_Bcast(&tosses, 1, MPI_LONG_LONG_INT, 0, MPI_COMM_WORLD);

    // Cada proceso hace su parte
    local_tosses = tosses / size;

    // Inicializar generador de números aleatorios distinto en cada proceso
    ...
    // Reducimos sumando todos los locales
    MPI_Reduce(&local_in_circle, &total_in_circle, 1, MPI_LONG_LONG_INT,
               MPI_SUM, 0, MPI_COMM_WORLD);

    // El proceso 0 calcula e imprime pi
    if (rank == 0) {
